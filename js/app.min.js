...
const menuCategoryArrows = document.querySelectorAll('.menu__item1 .menu__arrow_a');
if (menuCategoryArrows.length) {
	menuCategoryArrows.forEach((arrow) => {
		arrow.addEventListener('click', function (e) {
			if (window.innerWidth > 767) return;
			e.preventDefault();
			e.stopPropagation();
			const currentItem = arrow.closest('.menu__item1');
			if (!currentItem) return;
			const allItems = document.querySelectorAll('.menu__item1._active');
...
export function pageNavigation() {
	...
	// data-goto-header - учитовать header
	//data-goto-speed - скорость
	document.addEventListener("click", pageNavigationAction);
	function pageNavigationAction(e) {
		const targetElement = e.target;
		if (targetElement.closed(`[data-goto]`)) {
			const gotoLink = targetElement.closed('[data-goto]');
			const noHeder = gotoLink.hasAttribute('data-goto-header') ? true : false;
			const gotoSpeed = gotoLink.dataset ? gotoLink.dataset.gotoSpeed : '500';
			gotoBlock(`#${gotoLink.dataset.goto}`, noHeder, gotoSpeed);
			e.preventDefault();
		}
	}
}
...
document.addEventListener("DOMContentLoaded", function () {
	...
	const list = pagging.querySelector(".pagging__list");
	const prev = pagging.querySelector("#prev");
	const next = pagging.querySelector("#next");

	const totalPages = +pagging.dataset.total || 1;
	const path = window.location.pathname;

	// если 1 страница — скрываем пагинацию
	if (totalPages <= 1) {
		pagging.style.display = "none";
		return;
	}

	// база (например: /remont/santehnika)
	let base = path.replace(/\/page-\d+\/$/, "/").replace(/\/$/, "");

	// текущая страница (page-2, page-3...)
...
if (document.querySelector('.swiper')) {
	let swiperSlider = new Swiper('.swiper__slider', {
		/*
		effect: 'fade',
		autoplay: {
			delay: 3000,
	...
	let swiperDotts = document.querySelectorAll('.swiper__dotts .swiper-pagination-bullet');


	for (let index = 0; index < swiperImages.length; index++) {
		const swiperImage = swiperImages[index].querySelector('img').getAttribute('src');
		swiperDotts[index].style.backgroundImage = "url('" + swiperImage + "')";
	}
}
